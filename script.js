...
function launchFirework(){
  fireworks.push({x:Math.random()*canvas.width, y:canvas.height, targetY:Math.random()*canvas.height/2, exploded:false, particles:[]});
}

function heartPattern(x,y){
  const p=[];
  for(let t=0;t<Math.PI*2;t+=0.15){
    const r=25*(1-Math.sin(t));
    p.push({x,y,dx:r*Math.cos(t)*0.6,dy:-r*Math.sin(t)*0.6,life:100,color:"#f0f"});
  }
  return p;
}

function explode(fw){ fw.particles=heartPattern(fw.x,fw.y); }
...
function animateFireworks(){
  ...
  fireworks.forEach((fw,i)=>{
    if(!fw.exploded){
      ...
      if(fw.y<=fw.targetY){ fw.exploded=true; explode(fw); }
    } else {
...
/* ===== COUNTDOWN 10 DETIK ===== */
const newYear = Date.now() + 10000; // 10 detik test
...
function openAdmin(){
  if(prompt("Password admin:")==="admin2026"){
    ...
    data.forEach((w,i)=>{ wishList.innerHTML+=`<p>${i+1}. <b>${w.name}</b>: ${w.wish}</p>`; });
